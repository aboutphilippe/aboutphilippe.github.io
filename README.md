...
- **Committed:** `styles/Personal/` (**Personal** rules): avoid (filler, wordy openers, one, range/conclusions, hedging, clichés), formal transitions from ai-tells, encourage (direct address, rhetorical question), sentence length (suggestion: under 35 words per sentence). For DirectAddress and RhetoricalQuestion, a _section_ means all text between two headings (Tengo scripts in `styles/config/scripts/`).
